...
func (z *Reader) init(r io.ReaderAt, size int64) error {
	...
	for {
		...
		if err == ErrFormat || err == io.ErrUnexpectedEOF {
			dbg("%d: readDirectoryHeader %#v", len(z.File), end)
			break
		}
		if err != nil {
...
parseExtras:
	for extra := readBuf(f.Extra); len(extra) >= 4; { // need at least tag and size
		...
		case zip64ExtraID:
			...
			if needUSize {
				needUSize = false
				if len(fieldBuf) < 8 {
					dbg("zip64ExtraID.needUSize %q", fieldBuf)
					return ErrFormat
				}
			...
			if needCSize {
				needCSize = false
				if len(fieldBuf) < 8 {
					dbg("zip64ExtraID.needCSize %q", fieldBuf)
					return ErrFormat
				}
			...
			if needHeaderOffset {
				...
				if len(fieldBuf) < 8 {
					dbg("zip64ExtraID.needHeaderOffset %q", fieldBuf)
					return ErrFormat
				}
	...
	if needCSize || needHeaderOffset {
		dbg("neadCSize || needHeaderOffset")
		return ErrFormat
	}
...
func readDirectoryEnd(r io.ReaderAt, size int64) (dir *directoryEnd, err error) {
	...
	for i, bLen := range []int64{1024, 65 * 1024} {
		...
		if i == 1 || bLen == size {
			dbg("readDirEnd i==1 || %d == %d", bLen, size)
	...
	if o := int64(d.directoryOffset); o < 0 || o >= size {
		dbg("directoryOffset o=%d size=%d", o, size)
		return nil, ErrFormat
	}
	return d, nil
}
...
func readDirectory64End(r io.ReaderAt, offset int64, d *directoryEnd) (err error) {
	...
	if sig := b.uint32(); sig != directory64EndSignature {
		dbg("directory64EndSignature %d", sig)
		return ErrFormat
	}
